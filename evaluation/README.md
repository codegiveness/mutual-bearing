...
Focused refinement cases cover overbroad approval controls (`repair-control-side-effects`), a recipient without the sender's context (`recipient-aware-handoff`), and adequate versus inadequate existing facilities (`reuse-adequate-resource`, `reuse-insufficient-resource`). Compare the previous and revised payloads under the same cases and protocol, retain the no-guidance baseline, and include already-clear controls. Shared success establishes neither a behavioral gain nor general reliability.

For an end-to-end handoff exercise, give a second isolated session only the producer's actual generated `handoff.txt` and the recipient's next request—not the original conversation, supporting files, or an answer key. Inspect whether the recipient can finish with the supported facts, unresolved matters, and authority intact. Record both sessions and their connection; a complete-looking sender artifact alone does not establish recipient success.
